"""Compute per-token cross-compartment cosine similarity at layer 4 for each
(c, tr, wd) cell in the 8-256 rope sweep.
...
For each cell:
  ...
  4. Per-row cosine similarity between all c·(c-1)/2 compartment pairs.
     Report the mean off-diagonal of the resulting c×c matrix.

Cosine sim is rigid — not invariant under rotations — so it asks "do the
actual vectors point the same way" rather than "is the same subspace covered."

Output: cossim_sweep.json — dict keyed by "<group>/<dirname>" with fields
  {c, tr_eff, tr_raw, mode, wd, mean_off_diag_cossim, full_matrix_cossim, layer}
"""
...
def mean_cossim(x: torch.Tensor, y: torch.Tensor) -> float:
    ...
    Range [-1, 1]; 1 = identical direction per token, -1 = opposite.
    """
...
@torch.no_grad()
def cossim_for_run(run_dir: Path, batch_cpu: torch.Tensor, device: str,
                   capture_layer: int = DEFAULT_LAYER):
    """Returns dict with c, mean_off_diag_cossim, full_matrix_cossim (list of lists)."""
    ...
    for i in range(c):
        for j in range(i, c):
            v = mean_cossim(feats[i], feats[j]) if i != j else 1.0
            M[i, j] = v; M[j, i] = v
    if c < 2:
        mean_off = float("nan")
    else:
    ...
    return {"c": c, "mean_off_diag_cossim": mean_off, "full_matrix_cossim": M.tolist()}
...
def main():
    ...
    print(f"Canonical batch: {batch.shape}")
    ...
    for i, (g, d, cf) in enumerate(todo):
        ...
        try:
            res = cossim_for_run(d, batch, args.device, capture_layer=args.layer)
            if res is None:
